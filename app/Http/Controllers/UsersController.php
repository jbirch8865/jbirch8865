...
class UsersController extends Controller
{
    ...
    public function index(Request $request,int $company_id)
    {
        ...
        {
            Users::Query_Single_Table(array('username','active_status'),false,"WHERE `company_id` = '".$company_id."' AND `Active_Status` = '1'");
        }else
        {
            Users::Query_Single_Table(array('username','active_status'),false,"WHERE `company_id` = '".$company_id."'");
        }
        $Users = array();
        $count = 0;
